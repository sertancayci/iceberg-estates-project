...
function buildFormulaParts(agentIds, filterStatus, searchQuery, startDate, endDate) {
  let formulaParts = [];
  
  if (agentIds && agentIds.length > 0) {
    const agentFormula = agentIds.map(id => `FIND('${id}', ARRAYJOIN(agent_id)) > 0`).join(", ");
  ...
  if (searchQuery) {
    ...
    const searchFormula = [
      `REGEX_MATCH(LOWER({appointment_address}), '${searchTerm}')`,
      `REGEX_MATCH(LOWER(ARRAYJOIN({contact_name}, ', ')), '${searchTerm}')`,
      `REGEX_MATCH(LOWER(ARRAYJOIN({contact_surname}, ', ')), '${searchTerm}')`,
      `REGEX_MATCH(LOWER(ARRAYJOIN({contact_email}, ', ')), '${searchTerm}')`,
      `REGEX_MATCH(LOWER(ARRAYJOIN({contact_phone}, ', ')), '${searchTerm}')`
    ];
...
async function getTotalRecords(tableName, formula) {
  let apiUrl = `${API_URL}/${tableName}?view=Grid%20view&filterByFormula=${encodeURIComponent(formula)}`;
  let totalRecords = 0;

  try {
    let offset = '';
    do {
      const response = await axios.get(apiUrl + (offset ? `&offset=${offset}` : ''), { headers });
      totalRecords += response.data.records.length;
      offset = response.data.offset || '';
    } while (offset);
  } catch (error) {
...
const listRecords = async (
  ...
  let formulaParts = buildFormulaParts(agentIds, filterStatus, searchQuery, startDate, endDate);
  let combinedFormula = formulaParts.length > 0 ? `AND(${formulaParts.join(', ')})` : "";
  if (combinedFormula) {
    apiUrl += `&filterByFormula=${encodeURIComponent(combinedFormula)}`;
  }
