        ...
        <div class="modal-content">
            <div class="flex justify-center">
                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6  text-gray-600 mr-3 mb-3">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                </svg>
                <span class="text-gray-900 font-semibold">{{ modalMode === 'edit' ? 'Edit the' : 'Create an' }} Appointment</span>
            </div>
            <form class="w-full" @submit.prevent="submitForm">
                <div id="selectContactSection" class="w-full bg-white">
                    ...
                    <p v-if="errors && errors.selectedContact" class="error">{{ errors.selectedContact }}</p>
                </div>
                <div id="addressSection">
                    <AddressInput id="addressInput" v-model="form.address" @input="handleInput('address')" />
                    <p v-if="errors && errors.address" class="error">{{ errors.address }}</p>
                </div>
                <div id="selectAgentSection" class=" my-3">
                    <SelectBox label="Agents" :options="allAgents" v-model="form.selectedAgentIds" :multiple="true"
                        @handleInput="handleInput('selectedOptionId')" labelKey="agent_name"
                ...
                <div id="appointmentDateSection">
                    <DatePicker label="Appointment Date" ref-name="startDateInput" id="appointmentDate"
                        v-model="form.appointmentDate" @input="handleInput('appointmentDate')" required />
                    <p v-if="errors && errors.appointmentDate" class="error">{{ errors.appointmentDate }}</p>
                </div>

                <!-- Status Selection Section (only for edit mode) -->
                <div v-if="modalMode === 'edit'" id="statusSection" class="my-3">
                    <SelectBox :options="statusOptions" :modelValue="form.status"
                        @update:modelValue="form.status = $event" labelKey="text" valueKey="value" />
                </div>


                <div v-if="modalMode === 'edit'">
                    <RelatedAppointments :currentAppointmentId="currentAppointment.id"
                        :contactId="currentAppointment.contact_id" />
                </div>


                <div id="buttonsSections" class="w-full flex justify-end items-end mt-3">
                    <DynamicButton text="Cancel"
                        btnClass="bg-black hover:bg-gray-700 text-white font-semibold py-2 px-4 ml-2 rounded-md"
                        :action="close" />

                    <DynamicButton type="submit" :text="modalMode === 'edit' ? 'Save' : 'Create'"
...
export default {
    ...
    computed: {
        ...
        statusOptions() {
            ...
            return appointmentDate < now ?
                [{ id: 'completed', text: 'Completed' }, { id: 'cancelled', text: 'Cancelled' }] :
                [{ id: 'upcoming', text: 'Upcoming' }, { id: 'cancelled', text: 'Cancelled' }];
    ...
    methods: {
        validateForm() {
            ...
            // if (!this.form.selectedAgentIds) {
            //     this.errors.selectedAgentIds = 'Please select an agent.';
            //     isValid = false;
            // } else {
            //     this.errors.selectedAgentIds = '';
            // }
            ...
            if (!this.form.appointmentDate) {
                ...
                if (appointmentDate < now) {
                    this.errors.appointmentDate = 'Appointment date cannot be in the past.';
                    isValid = false;
                } else {
        ...
        handleInput(field) {
            console.log('Handling input:', field);
        ...
        selectContact(contact) {
            ...
            this.handleInput('selectedContactId');
        ...
        loadAppointment(appointment) {
            ...
            //add if agent_id has multiple values add form agentIds = appointment.agent_id
            ...
            const appointmentContact = this.contacts.find(contact => contact.id === appointment.contact_id[0]);
            this.form.selectedContact = appointmentContact;
            this.selectedContact = appointmentContact;
            console.log('this.form.selectedContact:', this.form.selectedContact.id);
        },
        ...
        fetchRelatedAppointments(appointmentId) {
            ...
            AirtableService.getRelatedAppointments(appointmentId)
                ...
                .catch(error => console.error('Error fetching related appointments:', error));
        },
        ...
        async submitForm() {
            ...
            if (this.validateForm()) {
                ...
                if (this.form.selectedAgentIds && this.form.selectedAgentIds.length > 0) {
                    appointmentData.agent_id = this.form.selectedAgentIds;
                    console.log('Agent IDs:', appointmentData.agent_id);
                }
                ...
                try {
                    if (this.modalMode === 'edit') {
                        ...
                        console.log('Appointment data:', appointmentData);
                        const updateAppointment = await AirtableService.updateAppointment(currentAppointmentId, appointmentData);
                        ...
                        const createdAppointment = await AirtableService.createAppointment(appointmentData);
